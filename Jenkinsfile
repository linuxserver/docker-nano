pipeline {
  ...
  environment {
    ...
    LS_REPO = 'docker-nano'
    CONTAINER_NAME = 'nano'
    DOCKERHUB_IMAGE = 'lsiodev/nano-beta'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/nano-beta-dev'
    PR_DOCKERHUB_IMAGE = 'lspipepr/nano-beta'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='false'
  }
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sL 'https://hub.docker.com/v2/repositories/nanocurrency/nano-beta/tags' |jq -r '.results[].name' |grep -Po "V\\d+\\d+.*" | head -n1
 ''',
            returnStdout: true).trim()
    ...
    // If this is a bin-beta build use live docker endpoints
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=bin-beta -v ${TEMPDIR}:/ansible/jenkins ghcr.io/linuxserver/jenkins-builder:latest 
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}' to bin-beta",\
             "type": "commit",\
